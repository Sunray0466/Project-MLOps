...
def fetch_kaggle(forced: bool=False) -> None:
    ...
    # download
    kg.api.authenticate()
    kg.api.dataset_download_files(dataset = kaggle_url, 
                                  path=f'{raw_dir}', 
                                  force=forced,
                                  unzip=True)
...
def preprocess_data() -> None:
    ...
    sort_order     = {v:f"{i:0>2}" for i,v in enumerate("ace two three four five six seven eight nine ten jack queen king".split())}
    ...
    targets_sort = sorted(targets_set-{"joker"}, key=sort_condition) + ["joker"] # remove joker, sort list and then add joker back in
